...
begin
  ...
  Length := Length + Count;
  Move(Value[StartIndex], FData[Length - Count], Count * SizeOf(Char));
  Result := Self;
end;
...
begin
  ...
  Move(Value, FData[Index], System.Length(Value) * SizeOf(Char));
  Result := Self;
end;
...
begin
  SetString(Result, PChar(FData), Length);
end;
...
begin
  ...
  begin
    Move(New, FData[Index], System.Length(New) * SizeOf(Char));
  end
  else
  begin
    OldLength := Length;
